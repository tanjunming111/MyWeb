...
var defaultDiaryData = [
    ...
    },
    {
        ...
        paragraphs: [
            ...
            "......",
            "发现一个问题，在本地使用这个网站时还比较流畅，但在浏览器上却是先渲染网页再渲染文字，在观感上不太好。我问了 Agent，它说是因为设置的时间戳是当前时间，这是为了即时更新修改来做的，从而导致了每次进入新网页时，URL更新了， Github 都要重新下载一次 JS 文件。于是我在本地钦定了一个时间戳，也以 JS 文件的形式保存，每次修改时时间戳+1。这样上传到 Github 之后，浏览器遇到的就一直是这个 URL，就不需要重复下载，然后保存时间戳的JS文件也不用重复下载，减少了文字渲染的时间。不过这个要调试的东西挺多的，弄了我好一会儿。",
        ]
    }
];
